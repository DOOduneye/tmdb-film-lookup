...
const Tv = ({ media, credits, similar, providers }) => {
...
const getServerSideProps = async (context) => {
    ...
    const res = await fetch(`https://api.themoviedb.org/3/tv/${tid}?api_key=${process.env.NEXT_PUBLIC_API_KEY_TMDB_V3}&language=en-US`);
    const media = await res.json();

    const creditsRes = await fetch(`https://api.themoviedb.org/3/tv/${tid}/credits?api_key=${process.env.NEXT_PUBLIC_API_KEY_TMDB_V3}&language=en-US`);
    const credits = await creditsRes.json();

    const similarRes = await fetch(`https://api.themoviedb.org/3/tv/${tid}/similar?api_key=${process.env.NEXT_PUBLIC_API_KEY_TMDB_V3}&language=en-US`);
    const similar = await similarRes.json();

    const providersRes = await fetch(`https://api.themoviedb.org/3/tv/${tid}/watch/providers?api_key=${process.env.NEXT_PUBLIC_API_KEY_TMDB_V3}`);
